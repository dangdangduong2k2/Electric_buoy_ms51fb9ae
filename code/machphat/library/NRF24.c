...
void nrf24_w_reg(uint8_t reg, uint8_t *dat, uint8_t size)
{
...
uint8_t nrf24_r_reg(uint8_t reg){
...
//	HAL_SPI_Transmit(&hspiX, &cmd, 1, spi_w_timeout);
//	HAL_SPI_Receive(&hspiX, &dat, size, spi_r_timeout);
	spi_transfer(cmd);
...
void nrf24_w_spec_reg(uint8_t *dat, uint8_t size){
	int i;
	for(i=0;i<size;i++)
	{
		spi_transfer(*(dat+i));
...
void nrf24_r_spec_reg(uint8_t *dat, uint8_t size){
	int i;
	for(i=0;i<size;i++)
	{
		*(dat+i)=spi_transfer(0x00);
	}
}
...
void nrf24_pwr_up(void){
	...
	dat = nrf24_r_reg(CONFIG);

	dat|= (1 << PWR_UP);

	nrf24_w_reg(CONFIG, &dat, 1);
}
...
void nrf24_tx_pwr(uint8_t pwr){
	...
	dat = nrf24_r_reg(RF_SETUP);

	dat &= 184;

	dat |= (pwr << RF_PWR);

	nrf24_w_reg(RF_SETUP, &dat, 1);
}
...
void nrf24_open_rx_pipe(uint8_t pipe, uint8_t *addr){
	...
	nrf24_w_reg(EN_RXADDR, &dat, 1);
}

void nrf24_cls_rx_pipe(uint8_t pipe){
	uint8_t dat = nrf24_r_reg(EN_RXADDR);
...
void nrf24_set_crc(uint8_t en_crc, uint8_t crc0){
	uint8_t dat = nrf24_r_reg(CONFIG);

	dat &= ~(1 << EN_CRC) & ~(1 << CRCO);

	dat |= (en_crc << EN_CRC) | (crc0 << CRCO);
...
uint8_t nrf24_read_bit(uint8_t reg, uint8_t b){

	if(nrf24_r_reg(reg) & (1 << b)){
		return 1;
	}
...
void nrf24_set_bit(uint8_t reg, uint8_t b, uint8_t val){
	uint8_t dat= 0;
...
void nrf24_dpl(uint8_t en){
	uint8_t feature = nrf24_r_reg(FEATURE);
...
void nrf24_set_rx_dpl(uint8_t pipe, uint8_t en){
	...
	uint8_t dynpd = nrf24_r_reg(DYNPD);
...
void nrf24_auto_ack_all(uint8_t ack){
	uint8_t enaa = nrf24_r_reg(EN_AA);
...
void nrf24_auto_retr_delay(uint8_t delay){
	uint8_t dat = nrf24_r_reg(SETUP_RETR);

	dat &= 15;

	dat |= (delay << ARD);

	nrf24_w_reg(SETUP_RETR, &dat, 1);
}
...
uint8_t nrf24_transmit(uint8_t *dat, uint8_t size){
	uint8_t cmd ;
	int i;
	ce_low();
...
void nrf24_transmit_no_ack(uint8_t *dat, uint8_t size){
	uint8_t cmd;
	int i;
	ce_low();
...
void nrf24_transmit_rx_ack_pld(uint8_t pipe, uint8_t *dat, uint8_t size){
	uint8_t cmd ;
	int i;
	if(pipe > 5){
		pipe = 5;
...
uint8_t nrf24_carrier_detect(void){
	return nrf24_r_reg(RPD);
}

uint8_t nrf24_data_available(void){

 	uint8_t reg_dt = nrf24_r_reg(FIFO_STATUS);
...
void nrf24_receive(uint8_t *dat, uint8_t size){
	uint8_t cmd;
	int i;
	cmd = R_RX_PAYLOAD;
